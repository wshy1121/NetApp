...
{	trace_worker();
	CUserManager::instance()->removeUser(pClientConnRead->clientId);
...
{	trace_worker();
	...
	std::shared_ptr<CUserInf> ptr(new CUserInf());
	pClientConn->userInf = ptr;
	...
	CUserManager::instance()->addUser(clientId, pClientConn->userInf.get());
	return pClientConn;
}
